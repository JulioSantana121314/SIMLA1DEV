...
### 2026-02-12/13 – Meta Messenger Integration Complete

**Objective**

Add Meta Messenger as second channel type to enable multi-platform messaging alongside Telegram. Support inbound/outbound messaging with end-to-end validation.

---

## Setup Process

### 1) Facebook Test Page Creation

- **Page created**: `Kamshub Test Bot` (unpublished)
- **Page visibility**: Unpublished (only admin visible, not public)
- **Admin profile**: Hidden from public "Page Owners" list
- **Purpose**: Testing Messenger integration without public exposure
- **Page ID**: `106322262820122`

### 2) Meta App Configuration

- **App created**: `Kamshub Messaging Dev`
- **App ID**: `1573714837226388`
- **Product added**: Messenger
- **App mode**: Development (only approved test users can interact)

**Page Access Token generated**:
- Location: Meta App Dashboard → Messenger → API Configuration → Access Tokens
- Permissions granted: `pages_messaging`, `pages_read_engagement`
- Token type: Page Access Token (unique per Page + Admin + App)
- Token stored in MongoDB `channels.credentials.pageAccessToken`

**Verify Token defined**:
- Custom string defined by developer (not generated by Meta)
- Example format: `kamshub_messenger_verify_abc123xyz`
- Used for webhook verification (GET request validation)
- Stored in MongoDB `channels.credentials.verifyToken`

### 3) MongoDB Channel Document

```json
{
  "_id": ObjectId("698ea415531e4521087cf08f"),
  "tenantId": "platform",
  "type": "messenger",
  "displayName": "Kamshub Test Messenger",
  "externalId": "106322262820122",
  "credentials": {
    "pageAccessToken": "EAA...",
    "verifyToken": "kamshub_messenger_verify_abc123xyz"
  },
  "isActive": true,
  "createdAt": "2026-02-13T04:09:00.000Z",
  "updatedAt": "2026-02-13T04:09:00.000Z"
}

Code Changes
Lambda Handler Updates
File: Single Lambda .mjs file (all-in-one architecture)

1) Added messengerWebhook function
Location: After telegramWebhook function

Responsibilities:

Handle GET requests (webhook verification)

Handle POST requests (inbound messages)

Validate verifyToken against MongoDB

Create/update conversations with externalThreadId = sender.id (PSID)

Save inbound messages with provider: "messenger"

Key differences from Telegram:

GET handler required (Telegram doesn't need verification)

PSID (Page-Scoped ID) as conversation identifier

No username in webhook payload (externalUsername: null)

message.mid as providerMessageId

2) Added sendMessengerMessage function
Location: After sendTelegramMessage function

Responsibilities:

Send messages via Messenger Send API

Endpoint: https://graph.facebook.com/v18.0/me/messages

Authorization: Bearer {pageAccessToken}

Return providerMessageId from response
...
Note: Supports both GET (verification) and POST (messages)

4) Modified sendConversationMessage function
Added channel type detection:

javascript
if (channel.type === 'telegram') {
  const res = await sendTelegramMessage({ channel, conversation, text });
  // ...
} else if (channel.type === 'messenger') {
  const res = await sendMessengerMessage({ channel, conversation, text });
  // ...
} else {
  return { statusCode: 400, body: JSON.stringify({ error: `Unsupported channel type` }) };
}
Webhook Configuration in Meta
Setup Steps
Navigate to: Meta App Dashboard → Messenger → Configuration → Webhooks
...
text
https://5ewhj564xzelbmiusggtp6qiva0mvott.lambda-url.us-east-2.on.aws/webhooks/messenger/698ea415531e4521087cf08f
Verify Token: kamshub_messenger_verify_abc123xyz (from MongoDB)

Click: "Verify and Save"

Meta sends GET request with hub.verify_token and hub.challenge

Lambda validates token and returns challenge
...
✅ messaging_postbacks (for future button interactions)

✅ messaging_optins (for future opt-in flows)

Subscribe page to app:

Select page from dropdown

Click "Subscribe" to link page events to webhook

End-to-End Validation
Test 1: Inbound (User → Kamshub)
Action: Sent message "Jelou2" to page via Messenger app
...
json
{
  "_id": ObjectId("698eb814c12f40c9a43f1ac5"),
  "tenantId": "platform",
  "channelId": "698ea415531e4521087cf08f",
  "externalThreadId": "26062642199999127",
  "participants": {
    "externalUserId": "26062642199999127",
    "externalUsername": null
  },
  "lastMessageAt": "2026-02-13T05:35:16.413Z",
  "createdAt": "2026-02-13T05:35:16.413Z",
  "updatedAt": "2026-02-13T05:35:16.413Z"
}
Message saved:
...
json
{
  ...
  "tenantId": "platform",
  "channelId": "698ea415531e4521087cf08f",
  "conversationId": ObjectId("698eb814c12f40c9a43f1ac5"),
  "direction": "inbound",
  "provider": "messenger",
  "providerMessageId": "m_p8ZUri5o1ZqMGXpwvPikWLHKfj9je0MMypVF1-dgFbATZNyVlex¥34-crgEaRllvV0F_",
  "text": "Jelou2",
  "createdAt": "2026-02-13T05:35:16.554Z"
}
Validation:

✅ Conversation created with PSID as externalThreadId
...
Test 2: Outbound (Kamshub → User)
Action:

bash
curl -X POST https://5ewhj564xzelbmiusggtp6qiva0mvott.lambda-url.us-east-2.on.aws/tenant/conversations/698eb814c12f40c9a43f1ac5/messages \
-H "Authorization: Bearer {idToken}" \
-H "Content-Type: application/json" \
-d '{"text": "¡Hola desde Kamshub vía Messenger! 🚀"}'
Result: ✅ Success
...
json
{
  "id": "698ebbc4f799a33b6525baf0",
  "tenantId": "platform",
  "channelId": "698ea415531e4521087cf08f",
  "conversationId": "698eb814c12f40c9a43f1ac5",
  "direction": "outbound",
  "provider": "messenger",
  "providerMessageId": "m_2wInHCse5dMCdpNemKgXVULHKfj9je0MWypVF1-dgFYQz_NHl7dBO7XlU7jV8PYYusYAMKpIXtLg9iQ0VI2lig",
  "text": "¡Hola desde Kamshub vía Messenger! ??",
  "createdAt": "2026-02-13T05:51:00.982Z"
}
Messenger app evidence:

✅ Message received in user's Messenger chat

✅ Displayed correctly with Kamshub page avatar

Validation:

✅ Message sent successfully via Messenger API

✅ providerMessageId returned from Meta API (not null)

✅ Message saved in MongoDB with direction: "outbound"

✅ Conversation lastMessageAt updated

Test 3: Inbox API (Multi-Channel)
Action:

bash
curl https://5ewhj564xzelbmiusggtp6qiva0mvott.lambda-url.us-east-2.on.aws/tenant/conversations \
-H "Authorization: Bearer {idToken}"
Result: ✅ Success
...
json
{
  "items": [
    {
      "id": "698eb814c12f40c9a43f1ac5",
      "channel": {
        "type": "messenger",
        "displayName": "Kamshub Test Messenger"
      },
      "externalThreadId": "26062642199999127",
      "lastMessagePreview": "¡Hola desde Kamshub vía Messenger! ??",
      "lastMessageAt": "2026-02-13T05:51:00.982Z"
    },
    {
      "id": "69669f0d59186f846d1b5d6e",
      "channel": {
        "type": "telegram",
        "displayName": "Tatiana"
      },
      "externalThreadId": "7759377832",
      "lastMessagePreview": "S",
      "lastMessageAt": "2026-01-14T21:11:35.712Z"
    }
  ]
}
Validation:

✅ Both channel types (Telegram + Messenger) appear in same inbox

✅ Conversations ordered by lastMessageAt descending

✅ Messenger conversation appears first (most recent)

✅ Each conversation includes complete channel metadata

Architecture Summary
Channel Types Supported
Channel	Type Value	Identifier	Username Support
Telegram	telegram	chat.id	✅ Yes (message.from.username)
Messenger	messenger	PSID (sender.id)	❌ No (always null)
Webhook Flows
Telegram:
...
Configured via setWebhook API call

Messenger:

GET for verification (returns hub.challenge)

POST for messages

Configured via Meta App Dashboard UI
...
Recipient: chat_id parameter

Messenger:

Endpoint: https://graph.facebook.com/v18.0/me/messages

Auth: Authorization: Bearer {pageAccessToken} header

Recipient: recipient.id in JSON body

Key Implementation Notes
1) Single Lambda Architecture
All functionality in one .mjs file:

No separate src/ folder structure

No Serverless Framework config

Direct Lambda deployment via .zip upload

Simplifies deployment and reduces cold start time

2) Provider Message ID Tracking
Both channels return unique message IDs:

Telegram: message.message_id (integer)

Messenger: message_id (string starting with m_)

Stored as providerMessageId in MongoDB for:

Message delivery confirmation

Future read receipts/status tracking

Debugging and support

3) Conversation Isolation
Each conversation uniquely identified by:
...
4) Emoji and UTF-8 Handling
Emojis stored with different encoding in MongoDB:

Input: "¡Hola 🚀"

Stored: "¡Hola ??"

Display: Render correctly in frontend with proper UTF-8 decoding

Security & Privacy
Test Page Configuration
Unpublished: Not visible in public search or feeds

Admin hidden: Profile not listed in "Page Owners"

Messenger functional: Users can still message via direct link (m.me/yourpage)

App Development Mode
Test users only: Only approved testers can interact with bot

No public access: Production requires App Review + Privacy Policy

Credentials stored: Tokens in MongoDB (dev environment)

⚠️ Production TODO: Migrate to AWS Secrets Manager

Token Security
pageAccessToken: Never exposed in logs or client-side code

verifyToken: Custom string, not derivable from other credentials

Tokens scoped to specific page (not user-level access)

Deployment Process
1) Code Update
Modified single Lambda .mjs file with:

messengerWebhook() function

sendMessengerMessage() function

Updated handler routing

Updated sendConversationMessage() logic
...
Meta App Dashboard → Messenger → Webhooks

Add Callback URL with channelId from MongoDB

Verify token matches MongoDB credentials.verifyToken

Subscribe page to webhook events

Status: ✅ Complete
Messenger integration fully functional:

✅ Webhook verification successful

✅ Inbound messages received and stored

✅ Outbound messages sent and delivered

✅ Multi-channel inbox working

✅ Provider message IDs tracked

✅ End-to-end validated with real messages

Next Steps:

 Implement frontend UI (React/Vue) with wireframes from 2026-01-13

 Add unreadCount tracking (mark-as-read endpoint)

 Implement SQS outbox pattern for async sending
